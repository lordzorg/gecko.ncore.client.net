...
namespace Gecko.NCore.Client.ObjectModel.V2
{
    ...
    public static class FunctionManagerExtensions
    {
        ...
        {
            var dokumentmaler = (DataSet)instance.Execute("HentDokumentmaler", journalpostId, dokumentbeskrivelseId, string.Join(",", dokumentmalTypeIds.Select(x => x.ToString(CultureInfo.InvariantCulture)).ToArray()));
        ...
        public static int OpprettTilgangsgruppe(this IFunctionManager instance, Journalpost journalpost, params int[] personIds)
        {
            ...
            return (int)instance.Execute("OpprettTilgangsgruppeHandler", 0, journalpost.Id.Value, string.Join(",", personIds.Select(x => x.ToString(CultureInfo.InvariantCulture)).ToArray()));
        }
    }
}
