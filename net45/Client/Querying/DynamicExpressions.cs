...
// ReSharper restore CheckNamespace
{
    ...
    public static class DynamicExpression
    {
        ...
        [SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures"), SuppressMessage("Microsoft.Design", "CA1004:GenericMethodsShouldProvideTypeParameter")]
        public static Expression<Func<TArg, TResult>> ParseLambda<TArg, TResult>(string expression, params object[] values)
        {
            return (Expression<Func<TArg, TResult>>)ParseLambda(typeof(TArg), typeof(TResult), expression, values);
        }
    ...
    internal class Signature : IEquatable<Signature>
    {
        ...
        {
            var signature = obj as Signature;
            return signature != null ? Equals(signature) : false;
        }
    }
    ...
    internal class ClassFactory
    {
        ...
        [SuppressMessage("Microsoft.Performance", "CA1810:InitializeReferenceTypeStaticFieldsInline", Justification = "Trigger lazy initialization of static fields")]
        static ClassFactory()
        {
        }
        ...
        private static FieldInfo[] GenerateProperties(TypeBuilder tb, DynamicProperty[] properties)
        {
            ...
            {
                var dp = properties[i];
                var fb = tb.DefineField("_" + dp.Name, dp.PropertyType, FieldAttributes.Private);
                var pb = tb.DefineProperty(dp.Name, PropertyAttributes.HasDefault, dp.PropertyType, null);
    ...
    public sealed class ParseException : Exception
    {
        ...
        public override string ToString()
        {
            return string.Format(CultureInfo.CurrentUICulture, Res.ParseExceptionFormat, Message, position);
        }
    ...
    internal class ExpressionParser
    {
        ...
        private void ProcessValues(object[] values)
        {
            ...
            {
                var value = values[i] as IDictionary<string, object>;
                if (i == values.Length - 1 && value != null)
                {
                    externals = value;
                }
                else
                {
                    AddSymbol("@" + i.ToString(CultureInfo.InvariantCulture), values[i]);
                }
            }
        }
...
#pragma warning restore 0219
        ...
        private Expression ParseIntegerLiteral()
        {
            ...
            {
                ulong value;
                if (!UInt64.TryParse(literal, out value))
                {
                    throw ParseError(Res.InvalidIntegerLiteral, literal);
                }
                NextToken();
                if (value <= (ulong)Int32.MaxValue)
                {
                    return CreateLiteral((int)value, literal);
                }
                if (value <= (ulong)UInt32.MaxValue)
                {
                    return CreateLiteral((uint)value, literal);
                }
                if (value <= (ulong)Int64.MaxValue)
                {
                    return CreateLiteral((long)value, literal);
                }
            ...
            {
                long value;
                if (!Int64.TryParse(literal, out value))
                {
                    throw ParseError(Res.InvalidIntegerLiteral, literal);
                }
                NextToken();
        ...
        private Expression ParseRealLiteral()
        {
            ...
            {
                float f;
                if (Single.TryParse(literal.Substring(0, literal.Length - 1), out f))
                {
                    value = f;
                }
            }
            ...
            if (value == null)
            {
                throw ParseError(Res.InvalidRealLiteral, literal);
            }
            NextToken();
            return CreateLiteral(value, literal);
        }

        private Expression CreateLiteral(object value, string literal)
        {
            var expr = Expression.Constant(value);
            literals.Add(expr, literal);
            return expr;
        }
        ...
        private Expression ParseIdentifier()
        {
            ...
            object keyword;
            if (keywords.TryGetValue(token.text, out keyword))
            {
                var type = keyword as Type;
                if (type != null)
                {
                    return ParseTypeAccess(type);
                }

                if (keyword == (object)keywordIt)
                {
                    return ParseIt();
                }
                if (keyword == (object)keywordIif)
                {
                    return ParseIif();
                }
                if (keyword == (object)keywordNew)
                {
                    return ParseNew();
                }
                NextToken();
            ...
            object symbol;
            if (symbols.TryGetValue(token.text, out symbol) || externals != null && externals.TryGetValue(token.text, out symbol))
            {
                var expr = symbol as Expression;
                if (expr == null)
                {
                    expr = Expression.Constant(symbol);
                }
                else
                {
        ...
        private Expression PromoteExpression(Expression expr, Type type, bool exact)
        {
            ...
            if (ce != null)
            {
                ...
                {
                    string literal;
                    if (literals.TryGetValue(ce, out literal))
                    {
                        ...
                        switch (Type.GetTypeCode(ce.Type))
                        {
                            ...
                            case TypeCode.Int64:
                            case TypeCode.UInt64:
                                value = ParseNumber(literal, target);
                                break;
                            case TypeCode.Double:
                            ...
                            case TypeCode.String:
                                value = ParseEnum(literal, target);
                                break;
                        }
                        if (value != null)
                        {
